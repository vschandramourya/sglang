...
from sglang.srt.speculative.eagle_utils import (
    EagleVerifyInput as SGLANG_EagleVerifyInput,
)
from sglang.srt.speculative.eagle_utils import (
    EagleVerifyOutput,
...
@dataclass
class EagleVerifyInput(SGLANG_EagleVerifyInput):
    ...
    def verify(
        ...
        predict_cpu = torch.empty(predict_shape, dtype=torch.int32)
        accept_index = torch.full(
            (bs, self.spec_steps + 1), -1, dtype=torch.int32, device="cuda"
        )
        accept_index_cpu = torch.full((bs, self.spec_steps + 1), -1, dtype=torch.int32)
        ...
        accept_index_cpu.copy_(accept_index, non_blocking=True)
        predict_cpu.copy_(predict, non_blocking=True)
        has_finished = False
        ...
        if not has_finished:
            ...
            if len(unfinished_accept_index) > 0:
                ...
                if page_size == 1 or self.topk == 1:
                    ...
                    batch.seq_lens_cpu.add_(accept_length + 1)
