...
class PhoenixDraftExtendCudaGraphRunner:
    def __init__(self, phoenix_worker: PhoenixWorker):
        # Parse args
        self.phoenix_worker = phoenix_worker
        if not hasattr(phoenix_worker, "model_runner"):
            # V2: EagleDraftWorker
            self.model_runner = model_runner = phoenix_worker.draft_runner
            self.forward_mode = ForwardMode.DRAFT_EXTEND_V2
        else:
            self.model_runner = model_runner = phoenix_worker.model_runner
        ...
        self.phoenix_worker.draft_extend_attn_backend.init_cuda_graph_state(
            self.max_bs, self.max_num_token
        )
        self.seq_len_fill_value = (
            self.phoenix_worker.draft_extend_attn_backend.get_cuda_graph_seq_len_fill_value()
        )
        ...
        with torch.device(model_runner.device):
            ...
            if self.phoenix_worker.speculative_algorithm.is_eagle3():
    ...
    def capture_one_batch_size(self, bs: int, forward: Callable, stream_idx: int = 0):
        ...
        forward_batch = ForwardBatch(
            ...
            attn_backend=self.phoenix_worker.draft_extend_attn_backend,
            extend_seq_lens=extend_seq_lens,
            extend_seq_lens_cpu=extend_seq_lens_cpu,
            padded_static_len=self.padded_static_len,
        )

        self.phoenix_worker.draft_extend_attn_backend.init_forward_metadata_capture_cuda_graph(
            bs=bs,
        ...
        def run_once():
            ...
            ret = self.phoenix_worker.draft_model_runner.model.forward(
    ...
    def replay(self, forward_batch: ForwardBatch):
        ...
        self.phoenix_worker.draft_extend_attn_backend.init_forward_metadata_replay_cuda_graph(
            bs=bs,
