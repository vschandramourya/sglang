"""
Fixed Qwen3 Coder Detector with schema-based type conversion.

This module provides an improved version of the Qwen3CoderDetector that:
1. Uses schema-based type conversion for parameter values (like vLLM)
2. Has more robust regex for handling malformed XML
3. Adds explicit null value handling

Key differences from the original SGLang implementation:
- Parameter values are converted to their correct types (int, float, bool) based on tool schema
- More robust regex patterns handle missing </parameter> tags
- Explicit "null" string handling returns Python None
"""
...
class Qwen3CoderDetector(BaseFormatDetector):
    """
    Fixed detector for Qwen 3 Coder models with schema-based type conversion.
    ...
    - Robust regex for malformed XML
    ...
    def _convert_param_value(
        ...
        if param_name not in param_config:
            ...
            return self._safe_val_fallback(param_value)
    ...
    def _safe_val_fallback(self, raw: str) -> Any:
        """
    ...
    def parse_streaming_increment(
        ...
        while True:
            # If not in tool call and no start token, return normal text
            ...
            # In tool call, try to parse function name if not sent yet
    ...
    def _parse_and_stream_parameters(
        ...
        if new_params != self._current_parameters:
            ...
            if not self._current_parameters:
                # First parameter(s) - start JSON object
                items = []
